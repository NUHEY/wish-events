...
python3 - "$repo_dir" "$test_dir/link-tables.sql" <<'PYLINKS'
from pathlib import Path
import sys
source=(Path(sys.argv[1])/'supabase/migrations/20260827_add_resident_beta_tools.sql').read_text()
Path(sys.argv[2]).write_text(source[source.index('create table if not exists public.ra_link_hubs'):])
PYLINKS
"$pg_bin/psql" -X -h "$test_dir/socket" -d wish_events_security_test \
  -v ON_ERROR_STOP=1 -v link_tables_file="$test_dir/link-tables.sql" \
  -f "$repo_dir/tests/sql/link-hub-security.sql"

"$pg_bin/psql" -X -h "$test_dir/socket" -d wish_events_security_test \
  -v ON_ERROR_STOP=1 -f "$repo_dir/tests/sql/schedule-structure-security.sql"
