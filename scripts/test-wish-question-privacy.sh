...
python3 - "$repo_dir" "$test_dir/wisdom-definitions.sql" <<'PYWISDOM'
from pathlib import Path
import sys
r=Path(sys.argv[1])
w=(r/'supabase/migrations/20260828_wish_knowledge_and_resident_events.sql').read_text()
a=(r/'supabase/migrations/20260827_add_resident_beta_tools.sql').read_text()
Path(sys.argv[2]).write_text(w[w.index('create table if not exists public.wish_questions'):]+ '\n'+a[a.index('create table if not exists public.ra_questions'):a.index('create table if not exists public.ra_link_hubs')])
PYWISDOM
"$pg_bin/psql" -X -h "$test_dir/socket" -d wish_events_security_test \
 -v ON_ERROR_STOP=1 -v wisdom_definitions_file="$test_dir/wisdom-definitions.sql" \
 -f "$repo_dir/tests/sql/wish-question-privacy-security.sql"
