...
export function ShareQrTool() {
  ...
  return <div className="mx-auto max-w-2xl space-y-5"><Link href="/tools" className="inline-flex min-h-11 items-center gap-2 text-sm text-muted-foreground"><ArrowLeft className="h-4 w-4"/>{en?"Tools":"ツールへ"}</Link><header><h1 className="flex items-center gap-2 text-2xl font-bold"><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-lg bg-indigo-500/10 text-indigo-700 dark:text-indigo-300"><QrCode aria-hidden="true" className="h-5 w-5"/></span>{en?"Share a QR code":"共有QRコード"}</h1><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{en?"Turn an event or booking link into an image for posters and invitations.":"イベントや予約ページのリンクを、掲示物・案内に使える画像にします。"}</p></header><form onSubmit={e=>{e.preventDefault();void generate();}} className="space-y-3 rounded-2xl border border-border bg-card p-4 shadow-card"><Label htmlFor="share-url">{en?"Link to share":"共有するリンク"}</Label><Input id="share-url" type="url" inputMode="url" autoCapitalize="none" autoCorrect="off" required maxLength={2000} value={url} onChange={e=>{setUrl(e.target.value);setResult(null);setError("");}} placeholder="https://…" disabled={pending}/><Button disabled={pending} type="submit" className="w-full">{pending?(en?"Creating…":"作成中…"):(en?"Create QR code":"QRコードを作成")}</Button>{error&&<p role="alert" className="text-sm text-destructive">{error}</p>}</form>{result&&<section aria-live="polite" className="space-y-3 rounded-2xl border border-border bg-card p-4 shadow-card"><img src={result.image} alt={en?"QR code for the link below":"下のリンクを開くQRコード"} width={256} height={256} className="mx-auto h-auto w-full max-w-64 rounded-lg"/><p className="break-all text-xs text-muted-foreground">{result.url}</p><a href={result.image} download="wish-share-qr.png" className={buttonVariants({variant:"outline",className:"w-full"})}><Download className="h-4 w-4"/>{en?"Save image":"画像を保存"}</a><p className="text-xs leading-relaxed text-muted-foreground">{en?"Login and access permissions still apply. Creating a QR code does not make a private page public.":"読み取り後もログインや閲覧権限が必要です。QRコードを作っても、非公開ページが公開されることはありません。"}</p></section>}</div>;
}
