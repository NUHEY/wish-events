...
export function ResidentEventForm({ userId }: { userId: string }) {
  ...
  return <form action={action} className="space-y-5"><input type="hidden" name="image_url" value={imageUrl} />
    ...
    <section className="space-y-3 rounded-2xl border border-border bg-card p-4 shadow-card sm:p-5"><div><h2 className="font-bold">募集画像</h2><p className="mt-1 text-xs text-muted-foreground">写真をドロップするか、既定デザインを選べます。</p></div><ImageDropzone value={imageUrl} onFile={upload} disabled={uploading} label="写真を追加" hint="JPG・PNG・WebP／10MB以下" compact /><EventImagePreview thumbnailUrl={imageUrl} posterUrl={imageUrl} />{uploadError && <p className="text-sm text-destructive">{uploadError}</p>}<div className="grid grid-cols-4 gap-2">{DEFAULT_EVENT_PRESETS.map((preset) => <button key={preset.id} type="button" onClick={() => setImageUrl(preset.url)} aria-pressed={imageUrl === preset.url} className={cn("overflow-hidden rounded-xl border-2", imageUrl === preset.url ? "border-primary ring-2 ring-primary/10" : "border-transparent")}><Image src={preset.url} alt={preset.label} width={160} height={100} className={cn(EVENT_CARD_RATIO_CLASS, "w-full object-cover")} /><span className="block truncate bg-secondary/40 px-1 py-1 text-[9px] font-semibold">{preset.label}</span></button>)}</div></section>
