...
export function ProfileCoverUploader({ initialUrl }: { initialUrl: string | null }) {
  ...
  return <div className="grid gap-2"><PendingFeedback active={pending} label="カバー画像を更新しています…" /><ImageDropzone value={url} onFile={upload} disabled={pending} label="プロフィールのカバー画像（任意）" />{url && <Button type="button" variant="ghost" size="sm" className="w-fit" disabled={pending} onClick={remove}><X className="h-3.5 w-3.5" />削除</Button>}<p className="text-xs text-muted-foreground">国際交流や寮生活が伝わる横長の写真がおすすめです。</p></div>;
}
