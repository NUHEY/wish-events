...
function Content({ variant }: { variant: LoadingVariant }) {
  switch (variant) {
    ...
    case "talks":
      return <div className="mx-auto max-w-3xl space-y-4"><Block className="h-8 w-24" /><Block className="h-11 w-full rounded-xl" />{Array.from({ length: 5 }, (_, key) => <div key={key} className="flex items-center gap-3 px-1 py-3 sm:px-3"><Block className="h-14 w-14 shrink-0 rounded-full" /><div className="min-w-0 flex-1 space-y-2"><Block className="h-4 w-1/2" /><Block className="h-3 w-3/4" /></div></div>)}</div>;
    case "chat":
