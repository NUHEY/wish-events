...
import { MANAGEMENT_GROUPS, MANAGEMENT_MODULES, canManage, type ManagementAccess } from "@/lib/management-permissions";
...
export function DashboardNav({ access }: { access: ManagementAccess }) {
  ...
  function link(href: string, label: string, overview = false) {
    const active = overview ? pathname === href : pathname === href || (href !== "/dashboard" && pathname.startsWith(href + "/"));
    return <Link key={href + label} href={href} prefetch={false} onClick={() => setOpen(false)} aria-current={active ? "page" : undefined} className={cn("flex min-h-11 items-center justify-between gap-2 rounded-lg px-3 py-2 text-sm font-medium leading-relaxed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring", active ? "bg-primary text-primary-foreground" : "hover:bg-secondary")}><span>{label}</span><ChevronRight aria-hidden="true" className="h-4 w-4 shrink-0 opacity-60" /></Link>;
  }
  const groups = <div className="space-y-4">{link("/dashboard", en ? "Overview" : "概要", true)}{MANAGEMENT_GROUPS.map(group => {
    const items = modules.filter(module => module.group === group.key);
    if (!items.length && !(group.key === "people" && access.isRa)) return null;
    return <section key={group.key}><h2 className="mb-1 px-3 text-xs font-bold text-muted-foreground">{en ? group.en : group.ja}</h2><div className="space-y-1">{items.map(module => link(module.key === "events" ? "/dashboard#managed-events" : module.href, en ? module.en : module.ja))}{group.key === "people" && access.isRa && <>{link("/dashboard/permissions", en ? "Staff permissions" : "関係者の権限")}{link("/dashboard/ra-rooms", en ? "RA appointments" : "RAの任命・部屋")}</>}</div></section>;
  })}</div>;
  return <>
    <div className="md:hidden"><button ref={trigger} type="button" onClick={() => setOpen(true)} aria-haspopup="dialog" aria-expanded={open} className="flex min-h-11 w-full items-center gap-3 rounded-xl border border-border bg-card p-3 text-left"><Menu className="h-5 w-5 shrink-0 text-primary" /><span className="min-w-0 flex-1 text-sm font-semibold">{title}</span><span className="shrink-0 text-xs text-muted-foreground">{en ? "Menu" : "メニュー"}</span></button></div>
