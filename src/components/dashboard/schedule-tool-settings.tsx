  ...
  return (
    <section className="space-y-4 rounded-2xl border border-border bg-card p-4 shadow-card sm:p-5">
      ...
      <div className="grid gap-3 sm:grid-cols-4">
        <div className="grid gap-1.5"><Label htmlFor="schedule-default-start">開始時刻</Label><Input id="schedule-default-start" type="time" value={startTime} onChange={(event) => setStartTime(event.target.value)} /></div>
        <div className="grid gap-1.5"><Label htmlFor="schedule-default-end">終了時刻</Label><Input id="schedule-default-end" type="time" value={endTime} onChange={(event) => setEndTime(event.target.value)} /></div>
        <div className="grid gap-1.5"><Label htmlFor="schedule-default-slot">1枠の長さ</Label><Select id="schedule-default-slot" value={slotMinutes} onChange={(event) => setSlotMinutes(Number(event.target.value) as 15 | 30 | 60)}><option value={15}>15分</option><option value={30}>30分</option><option value={60}>60分</option></Select></div>
        <div className="grid gap-1.5"><Label htmlFor="schedule-default-days">期間の上限</Label><Input id="schedule-default-days" type="number" min={3} max={31} value={maxDays} onChange={(event) => setMaxDays(Number(event.target.value))} /><p className="text-[10px] text-muted-foreground">3〜31日</p></div>
      </div>
