...
import { sendRaBroadcastNotification, type BroadcastTarget } from "@/actions/notifications";
...
export function BroadcastNotificationForm({ residents }: { residents: BroadcastResident[] }) {
  ...
  }, [floor, link, message, mode, role, selectedIds]);
  ...
  async function handleSend() {
    ...
    startTransition(async () => {
      const result = await sendRaBroadcastNotification({ message, link, target: buildTarget(), broadcastId });
      if (result.error) {
  ...
  return <div className="space-y-6"><PendingFeedback active={pending} label="通知を送信しています…" /><section className="rounded-2xl border border-border bg-card p-4 shadow-card sm:p-5"><div className="grid grid-cols-2 gap-2 sm:grid-cols-4">{([ ["all", "全寮生"], ["floor", "フロア"], ["role", "役割"], ["individual", "個別"] ] as const).map(([value, label]) => <button key={value} type="button" onClick={() => setMode(value)} className={cn("rounded-xl border px-3 py-2.5 text-sm font-semibold active:scale-[0.98]", mode === value ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background")}>{label}</button>)}</div>
