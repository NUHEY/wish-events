...
export function LinkHubEditor({ initialHub, initialItems, defaultSlug, defaultTitle }: { initialHub: LinkHubInitial; initialItems: LinkHubItemInput[]; defaultSlug: string; defaultTitle: string }) {
  ...
  function addItem() {
    setItems((current) => current.length >= 30 ? current : [...current, { editorId: crypto.randomUUID(), title: "", url: "https://", description: "", icon: "link", enabled: true }]);
  }
  ...
  async function save() {
    ...
    try {
      ...
      toast.error("保存結果を確認できませんでした。接続を確認して、もう一度お試しください。");
    } finally {
