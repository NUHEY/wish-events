...
export function AnnouncementComments({
  ...
  function submit() {
    ...
    startTransition(async () => {
      const result = await addAnnouncementComment(announcementId, body, replyTo?.id);
  ...
  function like(commentId: string, likedByMe: boolean, likeCount: number) {
    ...
    toggleAnnouncementCommentLike(commentId, announcementId, likedByMe).catch(() => {
  ...
  async function handleDelete(comment: Comment) {
    ...
    startTransition(async () => {
      const result = await deleteAnnouncementComment(comment.id, announcementId);
      if (result?.error) {
        setError(result.error);
        setDeletedIds((current) => {
          const next = new Set(current);
