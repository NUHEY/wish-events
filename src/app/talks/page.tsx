...
import { Building2, MessageCircle, UserPlus, Users } from "lucide-react";
...
export default async function TalksPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  return <div className="mx-auto flex max-w-2xl flex-col gap-4">
    <div className="flex flex-col items-stretch gap-3 border-b border-border pb-3 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-2xl font-bold">{dict.talks.title}</h1><p className="mt-0.5 text-sm text-muted-foreground">{dict.talks.subtitle}</p></div><TalksTabBar hasUnreadFloor={hasUnreadFloor} hasUnreadFriends={hasUnreadFriends} floorGroupState={floorGroupState} friendDmState={friendDmState} /></div>
    {tab === "events" ? <div className="divide-y divide-border/70">{rooms.map((event) => {
      ...
      return <Link key={event.event_id} href={`/talks/${event.event_id}`} className="group flex items-center gap-3 px-1 py-3 transition-colors active:bg-secondary/60 sm:hover:bg-secondary/35">
        ...
        <span className="min-w-0 flex-1"><span className="flex items-center gap-2"><span className={`truncate text-[15px] ${event.unread ? "font-bold" : "font-semibold"}`}>{locale === "en" && event.title_en ? event.title_en : event.title}</span>{event.unread && <span className="h-2 w-2 shrink-0 rounded-full bg-destructive" />}</span><span className={`mt-1 block truncate text-sm ${event.unread ? "font-medium text-foreground/75" : "text-muted-foreground"}`}>{preview}</span></span>
        <span className={`self-start pt-1 text-[11px] ${event.unread ? "font-semibold text-primary" : "text-muted-foreground"}`}>{compactTime(event.last_message_at, locale)}</span>
      </Link>;
    })}{rooms.length === 0 && <EmptyEvents label={dict.talks.noEventTalks} />}</div> : tab === "floor" ? <div className="divide-y divide-border/70">{floorThread ? <Link href="/talks/floor" className="group flex items-center gap-3 px-1 py-3 transition-colors active:bg-secondary/60 sm:hover:bg-secondary/35"><span className="relative flex h-[58px] w-[58px] shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary ring-2 ring-primary/15 ring-offset-2 ring-offset-background"><Building2 className="h-6 w-6" />{floorThread.unread && <span className="absolute -right-0.5 top-0 h-3.5 w-3.5 rounded-full border-2 border-background bg-destructive" />}</span><span className="min-w-0 flex-1"><span className="flex items-center gap-2"><span className={`truncate text-[15px] ${floorThread.unread ? "font-bold" : "font-semibold"}`}>{dict.talks.floorGroup.replace("{floor}", String(floorThread.floor_number))}</span>{floorThread.unread && <span className="h-2 w-2 shrink-0 rounded-full bg-destructive" />}</span><span className={`mt-1 block truncate text-sm ${floorThread.unread ? "font-medium text-foreground/75" : "text-muted-foreground"}`}>{floorThread.last_message_body?.trim() || dict.talks.floorInvitation}</span><span className="mt-1 flex items-center gap-1 text-[11px] text-muted-foreground"><Users className="h-3 w-3" />{dict.talks.memberCount.replace("{count}", String(floorThread.member_count))}</span></span><span className={`self-start pt-1 text-[11px] ${floorThread.unread ? "font-semibold text-primary" : "text-muted-foreground"}`}>{compactTime(floorThread.last_message_at, locale)}</span></Link> : <div className="flex flex-col items-center gap-2 py-14 text-center text-sm text-muted-foreground"><Building2 className="h-7 w-7" /><p>{dict.talks.floorProfileRequired}</p><Link href="/profile/edit" className="text-sm font-medium text-primary hover:underline">{dict.talks.checkProfile}</Link></div>}</div> : <div className="divide-y divide-border/70">{friendThreads.map((thread) => {
      const friend = thread.friend!; const preview = thread.last_message_type === "image" ? (thread.last_message_body?.trim() ? `📷 ${thread.last_message_body}` : dict.talks.imageReceived) : thread.last_message_body ?? dict.talks.startConversation;
      return <Link key={thread.friend_id} href={`/talks/friends/${thread.friend_id}`} className="group flex items-center gap-3 px-1 py-3 transition-colors active:bg-secondary/60 sm:hover:bg-secondary/35"><span className="relative shrink-0"><AvatarRing role={friend.role} size={58}><Image src={friend.avatar_url || DEFAULT_AVATAR_IMAGE_URL} alt="" width={58} height={58} className="h-[58px] w-[58px] rounded-full object-cover" /></AvatarRing>{thread.unread && <span className="absolute -right-0.5 top-0 h-3.5 w-3.5 rounded-full border-2 border-background bg-destructive" />}</span><span className="min-w-0 flex-1"><span className={`block truncate text-[15px] ${thread.unread ? "font-bold" : "font-semibold"}`}>{friend.full_name ?? dict.talks.residentFallback}</span><span className={`mt-1 block truncate text-sm ${thread.unread ? "font-medium text-foreground/75" : "text-muted-foreground"}`}>{preview}</span></span><span className={`self-start pt-1 text-[11px] ${thread.unread ? "font-semibold text-primary" : "text-muted-foreground"}`}>{compactTime(thread.last_message_at, locale)}</span></Link>;
    })}{friendThreads.length === 0 && <div className="flex flex-col items-center gap-2 py-14 text-center text-sm text-muted-foreground"><UserPlus className="h-6 w-6" /><p>{dict.talks.noFriends}</p><Link href="/directory" className="text-sm font-medium text-primary hover:underline">{dict.talks.viewDirectory}</Link></div>}</div>}
  </div>;
}

function EmptyEvents({ label }: { label: string }) { return <div className="py-14 text-center text-sm text-muted-foreground">{label}</div>; }
