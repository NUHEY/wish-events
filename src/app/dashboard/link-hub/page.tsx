...
export default async function DashboardLinkHubPage() {
  ...
  const { data: hub, error: hubError } = await supabase.from("ra_link_hubs").select("*").eq("owner_id", profile.id).maybeSingle();
  if (hubError) throw new Error("リンクページを読み込めませんでした。再読み込みしてください。");
  const { data: items, error: itemsError } = hub ? await supabase.from("ra_link_items").select("*").eq("hub_id", hub.id).order("position") : { data: [], error: null };
  if (itemsError) throw new Error("掲載リンクを読み込めませんでした。再読み込みしてください。");
